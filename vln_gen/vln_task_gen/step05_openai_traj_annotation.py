...
def build_messages_openai(episode: Dict, img_dir: str) -> List[Dict]:
    """构建发送给OpenAI的消息"""
    episode_id = episode["episode_id"]
    img_path = os.path.join(img_dir, episode_id)
    img_files = sorted(
    ...
    content.append({"type": "text", "text": "Here are the viewpoints and actions:"})
    
    # 添加每个视角
    for i, img_file in enumerate(img_files):
        # 添加视角文本
        content.append({"type": "text", "text": viewpoint_template.format(p=i)})
        ...
        if i + 1 < len(img_files) and i < len(episode["actions"]):
            ...
            content.append({"type": "text", "text": direction_template.format(action=action_text, p=i+1)})
    
    # 添加最后的总结指令
    content.append({"type": "text", "text": remaining_prompt.format(view_num=len(img_files))})
    
    return [{"role": "user", "content": content}]


def call_openai_api(messages: List[Dict], episode_id: str, client: OpenAI) -> str:
    """调用OpenAI API并处理响应"""
    try:
...
def process_episode(episode: Dict, img_dir: str, episode_id: str, client: OpenAI) -> Optional[str]:
    """处理单个episode，生成指令"""
    ...
    # 构建消息
    try:
        messages = build_messages_openai(episode, img_dir)
    except Exception as e:
    ...
    # 调用API
    try:
        instruction = call_openai_api(messages, episode_id, client)
...
def process_scene(args: argparse.Namespace, client: OpenAI) -> None:
    """处理单个场景的所有episodes"""
    ...
    for episode in instructions_data["episodes"]:
        ...
        # 生成指令
        instruction = process_episode(episode, img_dir, episode_id, client)
        if instruction:
...
def main():
    """主函数：参数解析和流程控制"""
    parser = argparse.ArgumentParser(description='VLN指令生成器 (OpenAI版本)')
    parser.add_argument("--vln_topo_path", type=str,
                       default="data/task_datasets/vln/hm3d_v2_l3mvn_refine_v1/train/content/episode_num_3100-3199",
                       help="VLN拓扑数据路径")
    ...
    parser.add_argument("--api_key", type=str, required=True,
                       help="OpenAI API Key")
    parser.add_argument("--base_url", type=str, default="https://www.chataiapi.com/v1",
