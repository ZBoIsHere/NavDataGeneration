...
CREATE TEMPORARY TABLE
	scene_ids AS (
		SELECT
			...
			COUNT(task_datasets_objectnav.id) AS id_count,
			STRING_AGG(task_datasets_objectnav.id, ',') AS id_list
		FROM
			public.task_datasets_objectnav
		WHERE
			task_datasets_objectnav.scene_type = '${scene_type}'
			...
			AND task_datasets_objectnav.recipe_tags @> ARRAY['${recipe_tag}']
		GROUP BY
			task_datasets_objectnav.scene_name
	);
